...
```
CONFLICT (content): Merge conflict in Calculadora.java
Automatic merge failed; fix conflicts and then commit the result.
```

Abrí `Calculadora.java`. Vas a ver algo así:
...
```bash
javac Calculadora.java && java Calculadora   # ¿compila y anda?
git status                                          # ¿queda algo en conflicto?
git add Calculadora.java                               # así marcás "resuelto"
...
- ¿Tirás una excepción: `throw new ArithmeticException("No se puede dividir por cero")`?
...
```bash
# 1. Editás Calculadora.java y dejás la versión final (igual que antes)
# 2. La probás
javac Calculadora.java && java Calculadora
