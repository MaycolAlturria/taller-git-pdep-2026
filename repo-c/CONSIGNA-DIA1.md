...
> Fijate que arriba ahora dice `tu-usuario/calculadora-c` y abajo, chiquito,
...
Abrí `calculadora.c` y, **justo debajo** del comentario `/* TODO: resta */`, escribí:
...
Y en `main`, **descomentá** la línea de la resta (sacale los `/*` y `*/`):
...
git add calculadora.c            # preparo el cambio
...
3. Hacé un cambio, no lo commitees, y ejecutá `git restore calculadora.c`.
